...
Deploying workloads to k8s cluster usually means deploying to shared envs.
...
- _Single instance of log agent._
...
Starting from k8s v1.12, each node may run [[https://kubernetes.io/docs/setup/cluster-large/][100 pods]]. Like any shared env, you may experience noisy neighborhood issue. One misbehaviors of one Pod will penalty all other pods in the same worker node. Want to disable logging for one problematic namespace? You can easily avoid emitting the log, but not the part of collecting log.
...
- _No gurantee for log transport latency_ 

Back-pressure; Slow disk create significant latencies for log agent
---------------------------------------------------------------------
